...
func _physics_process(delta):
	if status == true and Input.is_action_pressed("agarrar") and Global.Elevador_1 == 1:
		yield(get_tree().create_timer(0.3), "timeout")
		Global.Elevador_1 = 0
		Global.no_elevador = 0
		print(status," = Elevador para baixo")

	elif status == true and Input.is_action_pressed("agarrar") and Global.Elevador_1 == 0:
		yield(get_tree().create_timer(0.3), "timeout")
